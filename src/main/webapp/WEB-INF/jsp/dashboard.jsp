...
<div fit layout vertical iron-router>
    <core-drawer-panel touch-action data-drawer-panel responsiveWidth="768px" selected="main">
        <core-scroll-header-panel drawer>
            ...
            <div layout vertical>
                <core-menu class="nav">
                    <core-submenu label="Markets" icon="store">
                        <c:forEach var="market" items="${markets}">
                            <a href="#">
                                <paper-item tabindex="0">${market.name}</paper-item>
                            </a>
                        </c:forEach>
                    </core-submenu>
                    <core-submenu label="Products" icon="redeem">
                        <c:forEach var="product" items="${products}">
                            <a href="#">
                                <paper-item tabindex="0">${product.name}</paper-item>
                            </a>
                        </c:forEach>
                    </core-submenu>
                    <core-submenu label="Providers" icon="maps:local-shipping">
                        <c:forEach var="provider" items="${providers}">
                            <a href="#">
                                <paper-item tabindex="0">${provider.name}</paper-item>
                            </a>
                        </c:forEach>
                    </core-submenu>
                    <a href="#">
