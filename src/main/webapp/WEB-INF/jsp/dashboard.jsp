...
<%--</core-ajax>--%>
    ...
    <core-media-query query="max-width: 650px" queryMatches="{{phoneMode}}"></core-media-query>

    <div fit layout vertical iron-router>
        <core-drawer-panel id="navDrawerPanel" touch-action data-drawer-panel drawerWidth="{{drawerOpenMode ? '70px' : '256px'}}"
            ...
            <core-header-panel drawer>
                ...
                <div layout vertical>
                    <core-menu class="nav" selected="{{page}}" on-core-select="{{navDrawerClose}}" valueattr="data-category">
            ...
            <core-drawer-panel id="mainDrawerPanel" main responsive-width="650px"
                               drawerWidth="{{phoneMode ? '100%' : '33%'}}" narrow="{{_isMobile}}">


                <core-header-panel drawer>

                    <core-toolbar class="toolbar toolbar-primary">
                        <paper-icon-button role="button" tabindex="0" icon="menu" class="hidden-sm" on-tap="{{navDrawerToggle}}" aria-label="menu"></paper-icon-button>
                        <h3>{{page}}</h3>
                    </core-toolbar>
                    <div class="content" flex style="border-left: 2px solid;">
                        <core-pages selected="{{page}}" valueattr="data-category">
                            <section data-category="summary"></section>
                            <section data-category="market">
                                <paper-shadow z="1" class="card">
                                    <core-menu class="nav" selected="{{activeMarketId}}" valueattr="market-id">
                                        <template repeat="{{market in markets}}">
                                            <a href="#" market-id="{{market.id}}" on-tap="{{mainDrawerToggle}}">
                                ...
                                <paper-shadow z="1" class="card">
                                    <core-menu class="nav" selected="{{activeProductId}}" valueattr="data-category">
                                        <template repeat="{{product in products}}">
                                            <a href="#" data-category="{{product.id}}" on-tap="{{mainDrawerToggle}}">
                                                <paper-item style="border-bottom: 1px solid #ccc;" tabindex="0">
                                                    <div layout="" horizontal="" style="width: 100%;">
                ...
                <core-header-panel main>
                    <core-toolbar class="toolbar toolbar-primary">
                        <paper-icon-button icon="arrow-back" class="hidden-xs" on-tap="{{mainDrawerToggle}}"></paper-icon-button>
                        <h3>{{detailsTitle}}</h3>
                    </core-toolbar>
    ...
    dashboard.setActiveMarket = function(marketId) {
        dashboard.markets.forEach(function(market) {
            if(market.id == Number(marketId)) {
                ...
                dashboard.detailsTitle = market.name;
            }
        });
    };
    ...
    dashboard.setActiveProduct = function(productId) {
        dashboard.products.forEach(function(product) {
            if(product.id == Number(productId)) {
                ...
                dashboard.detailsTitle = product.name;
            }
        });
    };

    dashboard.page = "summary";
    dashboard.detailsTitle = "";

    dashboard.navDrawerToggle = function() {
        dashboard.$.navDrawerPanel.togglePanel();
    };

    dashboard.mainDrawerToggle = function() {
        dashboard.$.mainDrawerPanel.togglePanel();
    };

    dashboard.navDrawerClose = function() {
        dashboard.detailsTitle = "";
        dashboard.$.navDrawerPanel.closeDrawer();
    };
